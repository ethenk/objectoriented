...
public class Main {
    ...
    private void run() {
    	...
    	Sorter.insertionSort(studebtList, Sorter.SORT_ASCENDING);
    ...
    private void writeFile(ArrayList<Student> pStudentList) {
    	...
    	for(Student student: pStudentList) {
  // im not sure if this works or not, well have to check the output file at the end to see if it prints right
    		out.printf("%-16d %-20s %-15s %8.2f",pStudentList.get(student));
    	}
    	out.close();
    }
